    ...
    unsigned        flags)
{
    ...
    if (pattern_length == 0)
        pattern_length = strlen((char*)v_pattern);
                        ...
                        )
{
    ...
    *current_state = row | (current_matches<<24);
    ...
    /* If all strings are anchored at the beginning, and we hit
     * row=1, the default state for anchored matches, then we won't
     * ever be able to find the pattern */
    if (smack->is_anchor_begin && row == 1 && id == SMACK_NOT_YET_FOUND)
        id = SMACK_CANT_FIND;
