...
#define TIME64_CAST(ifmtime) ((unix_time64_t)ifmtime)
...
struct tm* gmtime64_r(const time64_t* timep, struct tm* result) {
        // On Windows, use the secure _gmtime64_s function
...
http_date_time_to_str (char * const s, const size_t sz, const unix_time64_t t)
{
  ...
  #endif
    return (__builtin_expect( (0 != gmtime_r(&t, &tm)), 1))
...
time_t
lighthttp_parse_date(const char * const ifmod, const uint32_t ifmodlen)
{
    struct tm ifmodtm;
    if (NULL == http_date_str_to_tm(ifmod, ifmodlen, &ifmodtm))
        return -1; /* date parse error */
    else {
        return portable_timegm(&ifmodtm);
    }
}
                             ...
                             const unix_time64_t lmtime)
{
    ...
    if (NULL == http_date_str_to_tm(ifmod, ifmodlen, &ifmodtm))
        ...
        const time_t ifmtime = portable_timegm(&ifmodtm);
